...
crx_folder_name='../chrome_ext/unzip_register'
pages_save_folder='../chrome_ext/pages_register'
log_file_path='../chrome_ext/log_register.txt'
register_fialed_log_path='../chrome_ext/log_register_failed.txt'
finished_file_path='../chrome_ext/finished_list_register.txt'
BUF_SIZE = 128*1024
...
def register_ext(driver,foldername,extension_id):
    # find sign up or register button
    ...
    save_page(origin_id,driver.execute_script("return document.body.innerHTML"),current_btn_count,page_url)
    # achieve register
    register_button=[]
    register_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'register')]")
    register_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Register')]")
    register_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'sign up')]")
    register_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Sign up')]")
    register_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Sign Up')]")
    register_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Join in')]")    
    print("find register button num",len(register_button))
    if len(register_button)==0:
        print("no register")
        return False
    # start register
    input_elements=driver.find_elements(By.XPATH,"//input")
    attr_list=['id','type','class','title','placeholder']
    field_list=['name','email','password']
    print("total input field",len(input_elements))
    for input_field in input_elements:
        if input_field.get_attribute('type')=='email':
            input_field.send_keys(register_profile['email'])
            continue
        if input_field.get_attribute('type')=='password':
            input_field.send_keys(register_profile['password'])
            continue
        if input_field.get_attribute('type')=='checkbox' or input_field.get_attribute('type')=='radio':
            input_field.click()
            continue
        fill_in=False
        attr_value_list=[input_field.get_attribute(i) for i in attr_list]
        attr_long_string=','.join(attr_value_list)
        for f in field_list:
            if f in attr_long_string:
                input_field.send_keys(register_profile[f])
                fill_in=True
                break
        if not fill_in:
            if input_field.get_attribute('type')=='text':
                input_field.send_keys(register_profile['name'])
            else:
                input_field.click()
    # once reuqire filling in before clicking
    try:
        register_button[0].click()
    except Exception as e:
        print(e)
        print("register failed",origin_id)
        return False
    ...
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Submit')]")
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'SUBMIT')]")    
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'register')]")
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Register')]")
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'sign up')]")
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Sign up')]")
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Sign Up')]")
    submit_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Join in')]")       
    if len(submit_button)==0:
        print("didn't find submit register button")
        return False
    try:
        submit_button[0].click()
        print("submit register info")
        page_url=str(driver.current_url)
        current_btn_count=len(driver.find_elements(By.XPATH,"//*"))
        save_page(origin_id,driver.execute_script("return document.body.innerHTML"),current_btn_count,page_url)
    except Exception as e:
...
def login_ext(driver,foldername,extension_id):
    ...
    login_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Login')]")
    login_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'LOGIN')]")    
    login_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'sign in')]")
    login_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Sign in')]")
    login_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'Sign In')]")
    login_button+=driver.find_elements(By.XPATH,"//button[contains(text(),'SIGN IN')]")     
    if len(login_button)==0:
        print("didn't find login register button")
        return False
    try:
...
def start_extension(driver,foldername,extension_id):
...
def time_record(foldername):
    try:  
        ...
        with open("traffic_data_register/request_all_in_one.txt",'a') as f:
            f.write("-----monitoring start-----"+str(datetime.datetime.now())+'\n')
            f.write(origin_id+'\n')
        with open("traffic_data_register/response_all_in_one.txt",'a') as f:
            f.write("-----monitoring start-----"+str(datetime.datetime.now())+'\n')
            f.write(origin_id+'\n')
        time.sleep(0.3)
        ...
        login_done=False
        try:
            register_done=register_ext(driver,foldername,extension_id)        
            login_done=login_ext(driver,foldername,extension_id)
        except:
            
            pass
        if not login_done or not register_done:
            print(extension_id,file=open(register_fialed_log_path,'a'))
        elif login_done:
            start_extension(driver,foldername,extension_id)
        else:
            driver.quit()
        #mitm.send_signal(signal.SIGINT)
        #mitm.terminate()
