...
def start_multi_thread_crawler(full_list_url_path,full_list_content_path):
    ...
    for url_item in url_list:
        ...
        unique_ids.append(id)
    uniques_ids=list(set(unique_ids))
    print("unique ids in total",len(uniques_ids))
    ...
    res_output.close()
    #TODO: fix bug, json format is wrong
    time.sleep(30)
    for idx in range(16):
        tmp_intermediate_file='./intermediate_files/thread_%s.json' % idx
        tmp_res_output=open(tmp_intermediate_file,'a')
        tmp_res_output.seek(-1, os.SEEK_END)
        tmp_res_output.truncate()
        tmp_res_output.write(']')
...
if __name__=='__main__':
    ...
    # partial_url_list=get_raw_list_from_url(firefox_sitemap_path)
    ...
    # browser_type='chrome'
    # full_list_content_path='./intermediate_files/thread_16.json'
    # get_fulllist_from_url(failed_list_url_path,full_list_content_path)

    #step5: combine all result
    # full_list_final_path='./chrome_fulllist_Oct_2023.json'
    # combine_all_list(full_list_final_path)

    #step6: after execution of get_source_code.py, get full list with available meta info and source code
